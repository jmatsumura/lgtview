...
if($indexed && &check_prep_status($md5)) {
	...
	# Done using the database at this point
	$dbh->disconnect();
}
...
#    }
    ...
    map {
        ...
        if($f[0]) {
			...
			# Pull annotation note from DB
			my $annot_note = $dbh->selectrow_array('SELECT cv.name FROM cv, curation WHERE curation.seq_id=? AND curation.cv_id=cv.id', undef, $f[0]);
